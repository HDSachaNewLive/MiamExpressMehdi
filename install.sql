...
CREATE TABLE IF NOT EXISTS `notify_list` (
  ...
  `date_ajout` TIMESTAMP    NULL     DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_notify_email` (`email`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- ============================================================
-- Log des envois (optionnel mais utile pour éviter les doublons)
-- ============================================================
...
ALTER TABLE `plats` ADD COLUMN `image_path` VARCHAR(512) DEFAULT NULL AFTER `prix`;
