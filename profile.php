...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_change_email'])) {
  ...
  if (empty($_POST['csrf_token']) || !fh_verify_csrf($_POST['csrf_token'])) {
    ...
    if ($is_fictif) {
        // Nettoyer les anciens tokens de vérification en attente
        $conn->prepare("UPDATE email_tokens SET used = 1 WHERE user_id = ? AND type = 'verify' AND used = 0")
             ->execute([$uid]);
        
        // Email fictif : on applique directement sans vérification
        ...
        // Email réel : on envoie un lien de confirmation vers le NOUVEL email
        $sent = fh_send_verify_email($conn, $uid, $user['nom_user'], $new_email, 'verify', $new_email);
        // On met aussi à jour email_fictif = 0 dès maintenant (en attente de vérif)
...
// Vérifier si un token de changement d'email est en attente
$pending_email = null;
...
<body>
  ...
  <div class="email-section">
    ...
    <!-- Alerte si email non vérifié (réel) -->
    ...
    <details class="change-email-details">
      ...
      <div class="change-email-form">
        <p class="change-email-info">
          ...
          Coche la case si cette nouvelle adresse est <strong>fictive ou temporaire</strong>.
        </p>
        <input type="email" id="new-email-input" placeholder="Nouvelle adresse email" autocomplete="email">

        <label class="checkbox-label-small" for="new-email-fictif">
          <input type="checkbox" id="new-email-fictif">
          <span>Mon nouvel email est fictif / temporaire (pas de confirmation envoyée)</span>
        </label>
  ...
  <form method="post" action="profile.php" class="form" enctype="multipart/form-data">
    ...
    <?php if (!empty($user['google_id'])): ?>
      <p>
        <small>ℹ️ Ton compte est lié à Google.</small><br>
...
document.getElementById('btn-change-email')?.addEventListener('click', async function() {
  ...
  const isFictif  = document.getElementById('new-email-fictif').checked ? 1 : 0;
...
/* Details / summary changement email */
...
/* Messages inline (succès/erreur AJAX) */
.inline-msg { margin-bottom: -20px; padding: 8px 12px; border-radius: 8px; font-size: 0.88rem; font-weight: 600; line-height: 1.4; }
.inline-msg.success { margin-top: 4px; margin-bottom: -3px; background: rgba(76,175,80,0.15); border-left: 3px solid #4CAF50; color: #2e7d32; }
.inline-msg.error   { margin-top: 4px; margin-bottom: -3px; background: rgba(255,77,77,0.15);  border-left: 3px solid #ff4d4d; color: #8b0000; }
#resend-msg.inline-msg.success { margin-top: 12px; }

/* ── Reste des styles profil ── */
.profile-preview {
...
.current-photo {
  ...
  border-radius: 50%; overflow: hidden; border: 4px solid #ff6b6b;
}
...
hr { margin-top: 20px; }
.btn {
  font-family: 'HSR', sans-serif; background: var(--accent); color: white;
...
.btn-public-profile {
  ...
  box-shadow: 0 6px 20px rgba(51,176,210,0.3); border: none;
  position: relative; overflow: hidden;
}
.btn-public-profile:hover {
  transform: translateY(-4px) scale(1.05);
  box-shadow: 0 10px 30px rgba(51,176,210,0.5);
  background: linear-gradient(135deg, #58edf5, #33b0d2); color: white;
}
.btn-del {
  padding: 15px 23px; font-size: 1.3rem; backdrop-filter: blur(15px);
  background: rgba(231,131,131,0.62); box-shadow: 0 8px 30px rgba(0,0,0,0.25);
...
.success {
  background: rgba(0,255,127,0.25); padding: 10px;
  border-radius: 10px; margin-bottom: 15px;
}
.error {
  background: rgba(255,77,77,0.25); padding: 10px;
  border-radius: 10px; margin-bottom: 15px;
}
ul li {
  ...
  align-items: center; transition: all 0.3s ease; color: rgba(53,53,53,0.85);
}
ul li:hover { background: rgba(255,255,255,0.22); transform: translateY(-2px); }
@keyframes fadeIn {
