...
try {
    ...
    if ($existing_by_google) {
        ...
        // mettre à jour le nom si changé sur Google
        $conn->prepare("UPDATE users SET nom_user = ?, google_photo = ? WHERE user_id = ?")
             ->execute([$google_name, $google_photo, $existing_by_google['user_id']]);

        _set_session($existing_by_google);
    ...
    if ($existing_by_email) {
        ...
        // ici ca lie le compte Google à ce compte FoodHub existant
        $conn->prepare("UPDATE users SET google_id = ?, google_photo = ?, nom_user = ? WHERE user_id = ?")
             ->execute([$google_id, $google_photo, $google_name, $existing_by_email['user_id']]);
    ...
    $_SESSION['success'] = "✅ Compte FoodHub créé avec succès via Google !";
    header('Location: home.php');
    exit;
