...
<body>
  ...
  <main class="container">
    ...
    <!-- gros title-->
    <h2 style="text-shadow: 2px 2px 4px rgba(255, 107, 107, 0.3); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><?= htmlspecialchars($restaurant['nom_restaurant']) ?> 🍽️</h2>
    
    <p class="container-desc"><?php if(!empty(htmlspecialchars($restaurant['description_resto']))){ echo nl2br(htmlspecialchars($restaurant['description_resto'])); } else { echo htmlspecialchars("Aucune description rédigée pour ce restaurant."); }?></p>
    ...
    <!-- si resto fermé, afficher bannière -->
    <?php if (!($restaurant['ouvert'] ?? 1)): ?>
      <div class="cuisine-fermee-banner">
        🚫 Ce restaurant est temporairement fermé - les commandes ne sont pas disponibles pour le moment.
      </div>
    <?php else: ?>
    <!-- Bouton impression du menu -->
    <?php if (!empty($plats)): ?>
      <div style="text-align:left; margin: 1.5rem 0; margin-top: 5px; margin-bottom: 10px;">
          <a class="btn-publish" href="menu_print.php?restaurant_id=<?= $restaurant_id ?>"
            target="_blank">
            🖨️ Imprimer la carte
          </a>
      </div>
        ...
        <?php foreach ($items as $p): ?>
          <div class="resto-card">
            <div class="plat-img-wrapper">
              <?php if (!empty($p['image_path']) && file_exists($p['image_path'])): ?>
                <img src="<?= htmlspecialchars($p['image_path']) ?>"
                     alt="<?= htmlspecialchars($p['nom_plat']) ?>"
                     class="plat-card-img">
              <?php else: ?>
                <img src="assets/foodhub-placeholder.png"
            ...
            <div class="add-to-cart" data-plat-id="<?= (int)$p['plat_id'] ?>">
              <input type="number" class="quantity-input" value="1" min="1" style="width:60px" required>
  ...
  <div class="resto-card comment-card" id="comment-<?= (int)$a['avis_id'] ?>" style="position:relative;">
    <div style="position:absolute; top:5px; right:10px; display:flex; gap:10px; z-index:10; pointer-events:auto;">
      ...
      <?php if (isset($_SESSION['user_id']) && $uid !== (int)$a['user_id']): ?>
        <button
          type="button"
          class="report-btn"
          data-avis-id="<?= (int)$a['avis_id'] ?>"
...
<!-- Modal signalement — hors du container, directement sur body -->
<div id="modal-signalement" style="position:fixed;inset:0;z-index:99999;
  ...
  <div style="background:rgba(255,255,255,0.97);backdrop-filter:blur(20px);
              border-radius:1.5rem;padding:2rem;width:90%;max-width:480px;
              box-shadow:0 10px 50px rgba(0,0,0,0.25);animation:fadeIn 0.25s ease;
              max-height:92vh;overflow-y:auto;">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;">
      <h3 style="margin:0;color:#ff6b6b;font-size:1.25rem;">🚩 Signaler ce commentaire</h3>
      <button onclick="fermerSignalement()" style="background:linear-gradient(135deg,#ff6b6b,#ff4d4d);border:none;border-radius:12px;width:40px;height:40px;font-size:1.2rem;cursor:pointer;transition:all 0.3s ease;color:white;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 15px rgba(255,77,77,0.3);font-weight:700;flex-shrink:0;" onmouseover="this.style.background='linear-gradient(135deg,#ff4d4d,#e03e3e)';this.style.transform='translateY(-2px) scale(1.05)';this.style.boxShadow='0 6px 20px rgba(255,77,77,0.5)'" onmouseout="this.style.background='linear-gradient(135deg,#ff6b6b,#ff4d4d)';this.style.transform='translateY(0) scale(1)';this.style.boxShadow='0 4px 15px rgba(255,77,77,0.3)'">❌</button>
    </div>
    <p style="color:#666;font-size:0.9rem;margin:0 0 1.2rem;">
      Ce signalement sera transmis à l'équipe FoodHub pour vérification. Merci de préciser la raison si possible.
    </p>
    <label style="display:block;font-weight:700;color:#333;margin-bottom:0.5rem;font-size:0.9rem;">
      Raison du signalement <span style="color:#aaa;font-weight:400;">(optionnel)</span>
    </label>
    <textarea id="signalement-raison" rows="4"
      placeholder="Ex : Contenu offensant, spam, hors-sujet…"
      style="width:100%;padding:0.8rem;border-radius:0.8rem;border:2px solid rgba(255,107,107,0.3);
            font-family:'HSR',sans-serif;font-size:0.95rem;resize:none;box-sizing:border-box;
            background:rgba(255,255,255,0.85);outline:none;transition:border-color 0.25s ease;"
      onfocus="this.style.borderColor='#ff6b6b'"
      onblur="this.style.borderColor='rgba(255,107,107,0.3)'"></textarea>
    <div style="display:flex;gap:0.8rem;margin-top:1.1rem;justify-content:flex-end;">
      <button type="button" onclick="fermerSignalement()"
        style="padding:0.7rem 1.4rem;border:none;border-radius:0.8rem;background:rgba(158,158,158,0.2);
              color:#555;font-weight:600;font-family:'HSR',sans-serif;cursor:pointer;transition:all 0.25s;"
        onmouseover="this.style.background='rgba(158,158,158,0.35)'"
        onmouseout="this.style.background='rgba(158,158,158,0.2)'">Annuler</button>
      <button type="button" id="signalement-submit"
        style="padding:0.7rem 1.5rem;border:none;border-radius:0.8rem;
              background:linear-gradient(135deg,#ff6b6b,#ff8c42);color:#fff;
              font-weight:700;font-family:'HSR',sans-serif;cursor:pointer;
              box-shadow:0 4px 14px rgba(255,107,107,0.35);transition:all 0.25s;">
...
.flash-message.hide { animation: flashOut 0.4s ease forwards; }
...
.comment-card > div[style*="position:absolute"] button:hover {
  background: rgba(211, 67, 67, 0.58); 
}
.comment-card p {
  margin-top: 10px;
  margin-bottom: 10px;
...
.comment-card .comment-meta {
  ...
  overflow: hidden;
  min-width: 0;
  /* laisser de la place aux boutons absolus en haut à droite */
  padding-right: 170px;
}
...
.btn [name="edit"]:hover{
    background: rgba(209, 66, 66, 0.56);
}
h4 {
  font-size: 1.25rem;
  margin-bottom: 19px;
...
.resto-card:hover .plat-card-img {
  transform: scale(1.05);
}
...
.cuisine-fermee-banner {
    ...
    font-weight: 700;
    font-size: 1.05rem;
    box-shadow: 0 4px 14px rgba(244,67,54,0.15);
}

.title-avis {
  font-size: 1.9rem;
  margin-bottom: 0px;
  text-shadow: 0 6px 14px rgba(244, 67, 54, 0.27);
  color: #ff6b6b;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.report-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      _signalementAvisId  = btn.dataset.avisId;
      _signalementRestoId = btn.dataset.restaurantId;
      document.getElementById('signalement-raison').value = '';
      const modal = document.getElementById('modal-signalement');
      modal.style.visibility = 'visible';
      modal.style.opacity    = '1';
      setTimeout(() => document.getElementById('signalement-raison').focus(), 120);
    });
  });

  document.getElementById('signalement-submit')?.addEventListener('click', async () => {
    const raison = document.getElementById('signalement-raison').value.trim();
    const btn    = document.getElementById('signalement-submit');
    btn.disabled    = true;
    ...
    try {
      ...
      if (data.success) {
        const reportBtn = document.querySelector(`.report-btn[data-avis-id="${_signalementAvisId}"]`);
        if (reportBtn) {
          reportBtn.textContent = '🚩 Signalé';
          reportBtn.classList.add('reported');
        }
        showMessage("✅ Signalement envoyé à l'administrateur.", 'success');
      } else {
  ...
  document.getElementById('modal-signalement')?.addEventListener('click', e => {
    if (e.target === document.getElementById('modal-signalement')) fermerSignalement();
  });

}); //fermeture du DOMContentLoaded existant

// Globales signalement
let _signalementAvisId  = null;
let _signalementRestoId = null;
